...
namespace CRM_TTV.Controllers
{
    ...
    public class CategoryBankingController : Controller
    {
        private CRM_TTVEntities db = new CRM_TTVEntities();
        int idCategoryType = 2;
        // GET: CategoryPhone
        public async Task<ActionResult> Index(string order, Int32? size = 10, Int32? page = 1)
        {
            ...
            if (!string.IsNullOrEmpty(order))
            {
                ...
                orderBy = order.Split('-');
                var tbCategories = db.tbCategories.Where(x=>x.idCategoryType == 2).OrderBy(orderBy[0] + " " + orderBy[1]).Skip(skip).Take(take);
            ...
            else
            {
                ...
                var tbCategories = db.tbCategories.Where(x => x.idCategoryType == 2).OrderByDescending(x => x.idCategory).Skip(skip).Take(take);
                ViewBag.Paging = Paging.Pagination(tbCategories.Count(), page, size);
                return View(await tbCategories.ToListAsync());
            }
        }
