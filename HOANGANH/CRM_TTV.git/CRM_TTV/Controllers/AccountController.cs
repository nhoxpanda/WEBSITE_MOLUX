...
namespace CRM_TTV.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Index(LoginViewModel model, string returnUrl)
        {
            ...
            {
                //mã hóa pass
                string passEncy = model.Password;
                var item = db.tbUsers.FirstOrDefault(u => u.email == model.Email && u.password == passEncy);
                string uRole = "";
                if (item != null)
                {
                    ...
                    {
                        uRole = db.tbRoles.FirstOrDefault(x => x.roleID == item.roleID).role; //quyền theo nhóm
                        Session["Menu"] = rcl.UserMenuRole(item.roleID.ToString());
                    }
                    else
                    {
                        uRole = item.userRole; //quyền đặc biệt
                    ...
                    var authTicket = new FormsAuthenticationTicket(1, item.email, DateTime.Now, DateTime.Now.AddMinutes(20), false, uRole);
        ...
        {
            //CHƯA CODE ĐĂNG KÝ, ĐĂNG KÝ XONG THÌ GỌI HÀM ĐĂNG NHẬP(account/index)
            if (ModelState.IsValid)
            {
