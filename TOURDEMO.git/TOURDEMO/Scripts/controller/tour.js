...
function checkEndTourDate() {
    var _songay = Number($("#tourDays").val());
    var startDateString = $("#startDateTour").val();
    var dateStart = new Date(startDateString);
    
    var dateEnd = new Date();
    if (_songay > 0) {
        dateEnd.setDate(dateStart.getDate() + (_songay - 1));
    }
    else {
        dateEnd.setDate(dateStart.getDate() + _songay);
    }
    ...
    var someFormattedDate = _dd + '/' + _mm + '/' + y;
    $("#endDateTour").val(someFormattedDate);
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data == 1) {
                ...
                $("#insert-quotation-edit").select2();
                $('#_priceTourEdit').number(true, 0);
