...
{
    public class PaymentMethodManageController : BaseController
    {
        // GET: PaymentMethod
        #region Init
        ...
        public PaymentMethodManageController(IGenericRepository<tbl_Dictionary> dictionaryRepository,
        ...
        {
            Permission(clsPermission.GetUser().PermissionID, 1145);
            var dictionary = _dictionaryRepository.GetAllAsQueryable().Where(p => p.DictionaryCategoryId == 31 && p.IsDelete == false).Select(p => new DictionaryViewModel
            {
                Id = p.Id,
                Name = p.Name
            });
        ...
        public async Task<ActionResult> Delete(FormCollection fc)
        {
            ...
            if (fc["listItemId"] != null && fc["listItemId"] != "")
            {
                ...
                if (listIds.Count() > 0)
                {
                    ...
                    {
                        return Json(new ActionModel() { Succeed = true, Code = "200", View = "", Message = "Xóa dữ liệu thành công !", IsPartialView = false, RedirectTo = Url.Action("Index", "PaymentMethod") }, JsonRequestBehavior.AllowGet);
                    }
                    else
                    {
