        ...
        public ActionResult FilterDate(DateTime start, DateTime end, int origin)
        {
            ...
            var model = _customerRepository.GetAllAsQueryable().AsEnumerable()
                ...
                .Where(p => (start == end ? start.ToString("dd-MM-yyyy") == p.CreatedDate.ToString("dd-MM-yyyy") : start <= p.CreatedDate && p.CreatedDate <= end)
                    ...
                    && p.IsDelete == false && p.IsTemp == false).OrderByDescending(p => p.CreatedDate)
                    .Select(p => new CustomerListViewModel() {
                        Id = p.Id,
                        Type = p.CustomerType == CustomerType.Organization ? "Doanh nghiệp" : "Cá nhân",
                        Address = p.Address,
                ...
                using (var stream = new MemoryStream())
                {
                    if (form["listItemId"] != null && form["listItemId"] != "")
                    {
                        ...
                        if (listIds.Count() > 0)
                        {
                            ...
                            filename = "[TRAVELPLUS] Thống kê khách hàng";
                    ...
                    else
                    {
                        ...
                        filename = "[TRAVELPLUS] Thống kê khách hàng " + (nguonden != "" ? "đến từ nguồn: " + nguonden : "") + " từ ngày " + Convert.ToDateTime(form["tungay"]).ToString("dd-MM-yyyy") + " đến " + Convert.ToDateTime(form["denngay"]).ToString("dd-MM-yyyy");
        ...
        public virtual void ExportCustomerToXlsx(Stream stream, IList<CustomerListViewModel> tours, string headername)
        {
            ...
            using (var xlPackage = new ExcelPackage(stream))
            {
                ...
                foreach (var t in tours)
                {
                    ...
                    worksheet.Cells[row, col].Value = t.Address + "," + (t.TagsId != null ? TRAVELPLUS.Utilities.LoadData.LocationTags(t.TagsId) : "");
                    col++;
